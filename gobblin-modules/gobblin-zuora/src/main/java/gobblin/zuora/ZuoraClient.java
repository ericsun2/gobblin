...
public class ZuoraClient {
  ...
  ZuoraClient(WorkUnitState workUnitState) {
  ...
  public CommandOutput<RestApiCommand, String> executePostRequest(final List<Command> cmds)
    ...
    try {
      return _retryer.call(new Callable<CommandOutput<RestApiCommand, String>>() {
        ...
        public CommandOutput<RestApiCommand, String> call()
            throws Exception {
          return executePostRequest(command);
        }
      });
    } catch (Exception e) {
  ...
  private CommandOutput<RestApiCommand, String> executePostRequest(Command command)
      throws IOException {
  ...
  public CommandOutput<RestApiCommand, String> executeRequest(List<Command> cmds)
      throws Exception {
    if (cmds == null || cmds.isEmpty()) {
      return null;
    }
    RestApiCommand.RestApiCommandType commandType = (RestApiCommand.RestApiCommandType) cmds.get(0).getCommandType();
  ...
  public CommandOutput<RestApiCommand, String> executeGetRequest(List<Command> cmds)
      throws Exception {
    HttpsURLConnection connection = null;
  ...
  public String getJobId(CommandOutput<?, ?> response)
      throws DataRecordException {
  ...
  public List<String> getFileIds(String jobId)
    ...
    while (!status.equals("completed")) {
      CommandOutput<?, ?> response = executeRequest(Collections.singletonList(cmd));
      Iterator<String> itr = (Iterator<String>) response.getResults().values().iterator();
      if (!itr.hasNext()) {
        throw new DataRecordException("Failed to get data from RightNowCloud; REST response has no output");
      }
  ...
  public BufferedReader getFileBufferedReader(String fileId)
      throws Exception {
    return new BufferedReader(new InputStreamReader(getRequestAsStream(getEndPoint("file/" + fileId))));
  }

  public InputStream getRequestAsStream(String urlPath)
      throws Exception {
  ...
  private String getEndPoint(String relativeUrl) {
    return _workUnitState.getProp(ConfigurationKeys.SOURCE_CONN_HOST_NAME) + relativeUrl;
  }
}
